...
class Node():
		...
		if joy.buttons[2] == 1 or self.go_up == True:
			...
			if self.find_height_mode == True:
				...
				if not self.dcc:  # acceleration mode
					...
					if self.speed > self.min_speed - 1:
						...
						print "xnya ",x
						print "dnya ",d
						print "real_dist",self.real_dist
						print "self dcc dist seharusnya sama kyk d", self.dcc_dist
						print "speed kalo pake teknik baru", x * d
			...
			if self.find_height_mode == True:
				...
				if not self.dcc: # acceleration mode
					...
					if self.speed > self.min_speed - 1:
						x = 10 / self.dcc_dist # determine a constant
						d = abs(self.real_dist - self.tablet_height)
						self.speed = int(round(x * d))+ (self.max_speed - self.min_speed)
						...
						print "dnya ",d
						print "real_dist",self.real_dist
						print "self dcc dist seharusnya sama kyk real dist", self.dcc_dist
						print "speed kalo pake teknik baru", x * d
						# for protection
						if self.speed > self.max_speed:
	...
	def speed_control(self,curr_dist,goal_dist):
		...
		# if total_distance > 7:
		# 	distance_to_acc = total_distance / 12
		# else:
		# 	distance_to_acc = total_distance / 5
		distance_to_acc = total_distance / (total_distance + 5)
		tolerance = 0.03
